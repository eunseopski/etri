...
def compute_euler_angles_from_rotation_matrices(rotation_matrices):
    ...
    out_euler[:, 0] = y * (1 - singular) + ys * singular  # pitch
    out_euler[:, 1] = x * (1 - singular) + xs * singular  # yaw
    out_euler[:, 2] = z * (1 - singular) + zs * singular  # roll

    return out_euler # radian

import torch

def compute_rotation_matrix_from_euler(euler_angles, normalized=False):
    """
    euler_angles shape: (B, 3)  또는 (B, 3, 1)
      - pitch(Y), yaw(X), roll(Z)
    - normalized=True면 0~1 정규화 상태라고 가정하고, denormalize + radian 변환까지 수행함
    return: (B, 3, 3)
    """
    if euler_angles.ndim == 3:
        euler_angles = euler_angles.squeeze(-1)
    assert euler_angles.ndim == 2 and euler_angles.shape[1] == 3

    if normalized:
        pitch = (euler_angles[:, 0] - 0.5) * 180
        yaw   = (euler_angles[:, 1] - 0.5) * 360
        roll  = (euler_angles[:, 2] - 0.5) * 180
        euler_angles = torch.stack([pitch, yaw, roll], dim=1)
        euler_angles = torch.deg2rad(euler_angles)  # degree → radian
    ...
    cx, sx = torch.cos(yaw),   torch.sin(yaw)
    cy, sy = torch.cos(pitch), torch.sin(pitch)
    cz, sz = torch.cos(roll),  torch.sin(roll)

    B = euler_angles.size(0)
    rot_mats = torch.zeros((B, 3, 3), device=euler_angles.device, dtype=euler_angles.dtype)

    # R = Rz @ Ry @ Rx
    rot_mats[:, 0, 0] = cz * cy
    rot_mats[:, 0, 1] = cz * sy * sx - sz * cx
    rot_mats[:, 0, 2] = cz * sy * cx + sz * sx

    rot_mats[:, 1, 0] = sz * cy
    rot_mats[:, 1, 1] = sz * sy * sx + cz * cx
    rot_mats[:, 1, 2] = sz * sy * cx - cz * sx

    rot_mats[:, 2, 0] = -sy
    rot_mats[:, 2, 1] = cy * sx
    rot_mats[:, 2, 2] = cy * cx

    return rot_mats, euler_angles
